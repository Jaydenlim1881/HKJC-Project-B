...
# Default SQLite database path; can be overridden via CLI argument
DEFAULT_DB_PATH = "hkjc_horses_dynamic_special.db"
...
def upsert_dynamic_stats(
    ...
    ''', (
        ...
        int(age) if age is not None and str(age).isdigit() else None,
        num_recent_runs,
        last_update
    ))
...
def extract_dynamic_stats(horse_url):
    template = "OtherHorse.aspx" if "OtherHorse.aspx" in horse_url else "Horse.aspx"
    log("INFO", f"Extracting using {template}: {horse_url}")
    ...
    try:
        ...
        # -----------------------------
        # PROFILE HEADER PARSING (AGE)
        # -----------------------------
        age = None
        birth_year = None
        header_section = soup.find(class_=re.compile("profile", re.I))
        header_text = header_section.get_text(" ", strip=True) if header_section else soup.get_text(" ", strip=True)
        if header_text:
            age_match = re.search(r'Age\s*:?\s*(\d+)', header_text, re.IGNORECASE)
            if age_match:
                try:
                    age = int(age_match.group(1))
                except Exception:
                    age = None
            birth_match = re.search(r'(19|20)\d{2}', header_text)
            if birth_match:
                try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process HKJC horse data")
    parser.add_argument(
        "--db-path",
    ...
    ensure_column_exists(DB_PATH, "horse_draw_pref", "ID", "INTEGER")
    ensure_column_exists(DB_PATH, "horse_draw_pref", "RaceCourse", "TEXT")
    ensure_column_exists(DB_PATH, "horse_draw_pref", "LastUpdate", "TIMESTAMP")
    ...
    for horse_id in horse_ids:
        ...
        primary_url = f"{base_url}Horse.aspx?HorseId={horse_id.strip()}"
        backup_url = f"{base_url}OtherHorse.aspx?HorseId={horse_id.strip()}"

        final_url = primary_url
        template_used = "Horse.aspx"
        ...
        try:
            ...
            log("INFO", f"Horse.aspx unavailable for {horse_id}: {e}; retrying OtherHorse.aspx")
            final_url = backup_url
            template_used = "OtherHorse.aspx"

        try:
            log("INFO", f"\nProcessing: {horse_id}")

            try:
                horse_data = extract_dynamic_stats(final_url)
            except Exception as e:
                if template_used == "Horse.aspx":
                    log("INFO", f"Horse.aspx parse failed for {horse_id}: {e}; trying OtherHorse.aspx")
                    final_url = backup_url
                    template_used = "OtherHorse.aspx"
                    horse_data = extract_dynamic_stats(final_url)
                else:
            ...
            log("INFO", f"Template used: {template_used}")
            ...
            if horse_data:
                ...
                try:
                    ...
                    if season:
                        ...
                        try:
                            ...
                            try:
                                header_cells = (
                                    horse_data["RawRows"][0]
                                    .find_parent("table")
                                    .find("tr")
                                    .find_all(["th", "td"])
                                )
                                for idx, cell in enumerate(header_cells):
                                    if cell.get_text(strip=True) == "Rtg.":
                                        rating_idx = idx
                                        break
                            except Exception:
                                pass

                            required_len = max(3, (rating_idx + 1) if rating_idx is not None else 3)
                            rows = [c for c in rows if len(c) >= required_len and c[2].get_text(strip=True)]

                            def _parse_iso(dmy):
                                s = dmy.strip()
                                try:
                                    return parse_hkjc_date(s).strftime("%Y-%m-%d")
                                except Exception:
                                    return None
                            ...
                            for c in rows:
                                ...
                                rating_txt = (
                                    c[rating_idx].get_text(strip=True)
                                    if rating_idx is not None and len(c) > rating_idx
                                    else ""
                                )
                                try:
                            ...
                            if parsed_valid:
                                parsed_valid.sort(key=lambda x: x[0])  # ascending by date
                                rating_start_career = parsed_valid[0][1]
                                ...
                                season_start = next(
                                    (
                                        rv
                                        for iso, rv in parsed_valid
                                        if _season_code(iso) == season
                                    ),
                                    parsed_valid[0][1],
                                )
                                rating_start_season = season_start

                                as_of_date, official_rating = parsed_valid[-1]
                            ...
                            log(
                                "ERROR",
                                f"Failed to upsert horse_rating for {horse_data.get('HorseID')}: {e}",
                            )
                ...
                sorted_raw_rows = sorted(
                    horse_data["RawRows"],
                    key=lambda row: parse_hkjc_date(
                ...
                weight_race_history = sorted(
                    horse_data["RawRows"],
                    key=lambda row: parse_hkjc_date(row.find_all("td")[2].get_text(strip=True)),
