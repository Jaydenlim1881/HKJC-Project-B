...
def parse_hkjc_date(date_str):
    """Parse a HKJC date string trying multiple formats.

    Attempts ``%d/%m/%y`` first, then ``%d/%m/%Y``. Raises ``ValueError`` if
    ...
    for fmt in ("%d/%m/%y", "%d/%m/%Y"):
        try:
            ...
            continue
    raise ValueError(f"Date '{date_str}' not in recognized HKJC format")
